...
$estado=iseguro($cone, $_POST['esta']);
$clientep=iseguro($cone,$_POST['clientepa']);
$iniped=fmysql(iseguro($cone,$_POST['inipeda']));
$finped=fmysql(iseguro($cone,$_POST['finpeda']));
	if(isset($clientep) && !empty($clientep) && isset($iniped) && !empty($iniped) && isset($finped) && !empty($finped) ){
		...
		?>
		<hr>
		<table id="dtventasa" class="table table-bordered table-hover"> <!--Tabla que Lista las vacaciones-->
		  <thead>
				<tr>
			...
			<tbody>
				...
				while($rcp=mysqli_fetch_assoc($cp)){
			            ...
			            <td><?php echo $rcp['nombre']?></td> <!--columna CLIENTE-->
									...
									<td>
										<a class="btn btn-info btn-xs" href="#" data-toggle="modal" data-target="#modal"><i class="fa fa-info-circle" onclick="detpeda(<?php echo $rcp['idventa']?>)"></i></a>
									</td> <!--columna DETALLE-->
								</tr>
								<?php
								}else{
