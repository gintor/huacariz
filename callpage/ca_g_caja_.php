									...
									if(mysqli_query($cone,"INSERT INTO docventa (numero, estado, idventa, idtipodoc) VALUES ($nd, 1, $idf, $dven);")){
										...
										if(mysqli_num_rows($cdv)>0){
											...
											while($rdv=mysqli_fetch_assoc($cdv)){
												...
												$vu=($rdv['subtotal']/($rdv['cantidad']*1.18));
												$bi=$vu*$rdv['cantidad'];
												$igv_item=$bi*0.18;
												
												$det.=$rdv['s_coduni']."|".n_22decimal($rdv['cantidad'])."|".$rdv['idproducto']."|-|".$rdv['nombre']."|".n_22decimal($vu)."|".n_22decimal($igv_item)."|1000|".n_22decimal($igv_item)."|".n_22decimal($bi)."|IGV|VAT|10|18.00|-|||||||-||||||".n_22decimal($rdv['subtotal'])."|".n_22decimal($bi)."|0.00|".PHP_EOL;
