...
if(vlogin($cone, $no, $id)){
	...
	$c=mysqli_query($cone, "SELECT idpersona, nombre FROM persona WHERE nombre LIKE '%$t%' OR numerodoc LIKE '%$t%' ORDER BY nombre ASC;");
	if(mysqli_num_rows($c)>0){
		while($r=mysqli_fetch_assoc($c)){
			$data[]=array('id'=>$r['idpersona'], 'text'=>html_entity_decode($r['nombre']));
		}
	}else{
