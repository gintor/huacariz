							...
							if($dven==3 && strlen($ndocp)!=11){
								...
								if(mysqli_query($cone,"UPDATE venta SET idcaja=$idca, estado=2, idtipopag=$tpag, descuento=$des, fecha='$ftven' WHERE idventa=$idf;")){
									...
									if(mysqli_query($cone,"INSERT INTO docventa (numero, estado, idventa, idtipodoc) VALUES ($nd, 1, $idf, $dven);")){
										...
										$cab="0101|$fven|$tven|-|$s_codloc|$tdocp|$ndocp|$nomcli|PEN|".n_22decimal($igv)."|".n_22decimal($rvv)."|".n_22decimal($pv)."|".n_22decimal($vdes)."|0.00|0.00|".n_22decimal($pv)."|2.1|2.0|";
										//generar .det
										...
										if(mysqli_num_rows($cdv)>0){
											...
											while($rdv=mysqli_fetch_assoc($cdv)){
												...
												$vu=(($rdv['subtotal']-$rdv['subtotal']*$des)/($rdv['cantidad']*1.18));
												$bi=$vu*$rdv['cantidad'];
												$igv_item=$bi*0.18;
												
												$det.=$rdv['s_coduni']."|".n_22decimal($rdv['cantidad'])."|".$rdv['idproducto']."|-|".$rdv['nombre']."|".n_22decimal($vu)."|".n_22decimal($igv_item)."|1000|".n_22decimal($igv_item)."|".n_22decimal($bi)."|IGV|VAT|10|18.00|-|||||||-||||||".n_22decimal($rdv['subtotal']-$rdv['subtotal']*$des)."|".n_22decimal($bi)."|0.00|".PHP_EOL;
										...
										$rvv=($pv/1.18);
										$rigv=$rvv*0.18;
										$tri="1000|IGV|VAT|".n_22decimal($rvv)."|".n_22decimal($rigv)."|";
										//Generamos ley
